...
exports.scan = (options) => {
  ...
  /* eslint-enable lodash/prefer-lodash-method */
    ...
    return Bluebird.try(() => {
      ...
      // After the file server usbboot phase completes, the USB device
      // will get disconnected and there is going to be time period
      // while we want for the block device to appear.
      // We want the USB device to "convert" to a block device rather
      // than disappearing for a while (for UX purposes), and a small
      // hack to do the trick is to add a delay right after the file
      // server phase completes.
      // TODO: This hack can be removed once we figure out a way to
      // reliably associate the USB device present at this point with
      // the block device that will come up afterwards.
      const BLOCKDEVICE_DELAY_MS = 5000
      return startFileServer(device, endpoint, options.readFile).delay(BLOCKDEVICE_DELAY_MS)
    }).return(device).catch({
